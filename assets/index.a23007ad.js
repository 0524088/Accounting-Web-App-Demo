...
function push({ name, instance, type }) {
  viewBackState.push({ name, instance, type });
}
...
function getAmountAlertColor(number) {
  ...
  const Colors = defaultStyle.color.bar.amountAlert;
...
function renderBarHTML(item, param, isGoal) {
  ...
  if ((item == null ? void 0 : item.unset) === true || isGoal == true && !item.goal) {
    ...
    progressHTML = `
            <div class="progress" style="height: 30px; position: relative;">
                ...
                <div class="h6 fw-bold w-100 text-center progress-content progress-item">
                    <span class="percent d-none" style="background-color: white;">${convert2ThousandsSeparator(current.amount)}</span>
                    <span class="number" style="background-color: white;">${convert2ThousandsSeparator(current.amount)}</span>
                </div>
            </div>
        `;
  } else {
    ...
    if (mode == "t-mode" || mode == "GoalDetailModal") {
      ...
      goalNumber = total;
      let bonusText = current.bonus != 0 ? `(+${current.bonus})` : "";
      progressHTML = `
                <div class="progress">
                    ...
                    <div class="h6 fw-bold w-100 text-center progress-content">
                        <span class="bg-white progress-item">    
                            ...
                            <!--<span style="color: ${getAmountAlertColor(value[0] + value[1])}">${current.amount}${bonusText}/${total}[${percent}%]</span>--->
                        </span>
                    </div>
                </div>
            `;
    ...
    if (mode == "it-mode") {
      ...
      if (goal.bonus == 0 && current.bonus != 0)
        bonusNumberText = `(<span style="color: ${getAmountAlertColor(value[4])}">+${current.bonus}</span>)`;
      if (goal.bonus != 0 && current.bonus != 0)
        bonusNumberText = `(<span style="color: ${getAmountAlertColor(value[4])}">+${current.bonus}/${goal.bonus}</span>)`;
      progressHTML = `
                <div class="progress">
                    <div class="progress-bar" role="progressbar" style="width: ${value[0]}%; background-color: ${getBarColor("amount")};" aria-valuenow="${value[0]}" aria-valuemin="0" aria-valuemax="100"></div>
                    <div class="progress-bar" role="progressbar" style="width: ${value[1]}%; background-color: ${getBarColor("bonus")};" aria-valuenow="${value[1]}" aria-valuemin="0" aria-valuemax="100"></div>
  ...
  return render2(name, mode, progressHTML, isGoal);
  function render2(name2, mode2, progressHTML2, isGoal2 = false) {
    let html;
    if (!isGoal2) {
      html = `
                <div class="items" data-sort-name="${name2}">
                    <div class="pb-3 pt-3 m-2">
                        <div class="d-flex align-items-center justify-content-start mb-2 gap-2">
                            <label class="form-label text-dark-gray h5 mb-1 item-long-word mw-100 ${mode2 == "i-mode" ? "text-center flex-grow-1" : ""}">${name2}</label>
      ...
      html = `
                <div class="m-2">
                    <div class="d-flex align-items-center justify-content-start mb-2 gap-2 unsortable">
                        <label class="form-label mt-3 h2 fw-bold">${name2}</label>
                    </div>
                    ${progressHTML2}
                </div>
            `;
...
function renderUserSettingContent(userSetting) {
  ...
  ShowChart.checked = userSetting.ShowChart;
...
async function initEventListener() {
  ...
  const ChartCollapse = document.querySelector("#chartCollapse");
  const ChartCollapseBtn = document.querySelector(".chart-collapse-button");
  ...
  new bootstrap.Collapse(ChartCollapse, { toggle: false });
  const SlideMenu_bs = new bootstrap.Offcanvas(SlideMenu);
  ...
  let DropdownTrigger;
  let AccountingDetailPointer = "";
  let Pickr2 = "";
  let showChart = UserSettingValue$1.ShowChart;
  ...
  SlideMenu.addEventListener("click", (e) => {
    ...
    const navLinks = SlideMenu.querySelectorAll(".nav-link");
    if (!selectedNavLink)
      return;
    ...
    if (((_c = selectedNavLink.dataset) == null ? void 0 : _c.tabTarget) == "#home") {
      setCollapseDisplay(ChartCollapse, showChart);
    }
    if (((_d = selectedNavLink.dataset) == null ? void 0 : _d.tabTarget) == "#setting") {
      setCollapseDisplay(ChartCollapse, false);
    }
    const subNav = document.querySelectorAll("nav [data-belong='sub']");
    subNav.forEach((sub) => {
      var _a2;
      if (((_a2 = selectedNavLink.dataset) == null ? void 0 : _a2.navBelong) == "sub")
  ...
  ChartCollapseBtn.addEventListener("click", (e) => {
    showChart = !showChart;
  });
  ChartCollapse.addEventListener("show.bs.collapse", (e) => {
    ChartCollapseBtn.classList.add("chart-collapse-button-up");
  });
  ChartCollapse.addEventListener("shown.bs.collapse", (e) => {
    ChartCollapse.classList.add("h-30vh");
    HomeContent.classList.remove("h-90vh");
    HomeContent.classList.add("h-60vh");
    ChartCollapse.classList.remove("no-animation");
  });
  ChartCollapse.addEventListener("hide.bs.collapse", (e) => {
    ChartCollapseBtn.classList.remove("chart-collapse-button-up");
    ChartCollapse.classList.remove("h-30vh");
    HomeContent.classList.remove("h-60vh");
    HomeContent.classList.add("h-90vh");
  });
  ChartCollapse.addEventListener("hidden.bs.modal", (e) => {
    ChartCollapse.classList.remove("no-animation");
  });
  ...
  SettingContent.addEventListener("change", async (e) => {
    ...
    switch (target.id) {
      ...
      case "NewItemPosition":
        ...
        break;
      case "ShowChart":
        UserSettingValue$1.ShowChart = target.checked;
        break;
      case "DebugMode":
  ...
  function setCollapseDisplay(collapseElement, display, useAnimation = false) {
    const collapse_bs = bootstrap.Collapse.getInstance(collapseElement);
    if (!useAnimation)
      collapseElement.classList.add("no-animation");
    if (display)
      collapse_bs.show();
    else
      collapse_bs.hide();
  ...
  async function removeItem() {
    await showAlert({
      ...
      if (confirm2.isConfirmed) {
        ...
        let nameArray = checkedArray.map((item) => item.closest(".items").querySelector("label").textContent);
  ...
  function doAfterRegisterAllEvent() {
    setCollapseDisplay(ChartCollapse, showChart);
  }
  doAfterRegisterAllEvent();
}
